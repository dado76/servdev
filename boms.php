	...
	$Secteur="";
		...
		if(isset($_GET['ppid'])){
			...
			while($rowLoader = $resLoader->fetch(PDO::FETCH_ASSOC)){
				...
				$Secteur=$rowLoader['Secteur'];
$Cubage=$rowLoader['Cubage'];
$PTAC=$rowLoader['PTAC'];
$PV=$rowLoader['PV'];
$CU=$rowLoader['CU'];
			}
...
<center><a href="addbom.php" id="myButton" class="fancybox fancybox.ajax"><i class="fa fa-plus"><br></i><br> Ajouter</a>
...
<?php
					...
					$str.="<thead><tr><th>ID</th>><th>Codification</th><th>Immatriculation</th><th>Secteur</th><th>Cubage</th><th>PTAC</th><th>PV</th><th>CU</th><th>Action</th></tr></thead><tbody>";
						while($row = $res->fetch(PDO::FETCH_ASSOC)){
							$str.="<tr><td><center>".$row['id']."</center></td>";
							$str.="<td>".$row['Codification']."</td>";
								$str.="<td>".$row['Immatriculation']."</td>";
              $str.="<td>".$row['Secteur']."</td>";
							  $str.="<td>".$row['Cubage']."</td>";
								  $str.="<td>".$row['PTAC']."</td>";
									  $str.="<td>".$row['PV']."</td>";
										  $str.="<td>".$row['CU']."</td>";
							$str.="<td><center><a class='fancybox fancybox.ajax' href='addbom.php?ppid=".$row['id']."' onclick='return update()'><img src = 'images/edit-icon.png' height='30' width='30' alt = 'edit' title = 'edit'/></a><a href='delbom.php?pid=".$row['id']."' onclick='return bura()' ><img src = 'images/edit_delete.png' height='30' width='30' alt = 'delete' title = 'delete'/></a></center></td></tr>";
						}
						echo $str;
